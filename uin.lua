...
function UILib.Window(titleA, titleB, gameName)
    ...
    local function getTabAPI(tabName)
        ...
        function api:SetButtonText(idx, text)
            local b = btns[idx]
            if b and b.isAct and b.lbl then b.lbl.Text = tostring(text or "") end
        end
        function api:GetButtonText(idx)
            local b = btns[idx]
            return b and b.isAct and b.lbl and b.lbl.Text or ""
        end
        ...
        function api:Dropdown(lbl, options, initIdx, cb)
            ...
            function ddApi:SetOptions(newOpts)
                ...
                for i = #b.optBgs + 1, #newOpts do
                    local oy2 = b.baseRY + b.ch + ((i - 1) * b.ch)
                    local obg = mkD(mkSq(uiX+b.rx,uiY+oy2,b.cw,b.ch,C.ROWBG,true,0,10,nil,4))
                    local oln = mkD(mkLn(uiX+b.rx,uiY+oy2+b.ch,uiX+b.rx+b.cw,uiY+oy2+b.ch,C.DIV,11,1))
                    local olb = mkD(mkTx(newOpts[i],uiX+b.rx+14,uiY+oy2+b.ch/2-6,11,C.WHITE,false,11))
                    obg.Visible=false; oln.Visible=false; olb.Visible=false
                    pcall(function() obg.Corner=2 end)
                    table.insert(b.optBgs,{bg=obg,ln=oln,lb=olb,ry=oy2,alpha=0,targetAlpha=0,hoverAlpha=0,targetHoverAlpha=0})
                end
                local maxO = #b.optBgs
                local trimmed = {}
                for i = 1, #newOpts do trimmed[i] = newOpts[i] end
                b.options = trimmed
                for i = 1, maxO do
                    if i <= #trimmed then
